...
async function geocodeAddress(address) {
  ...
  const res = await fetch(url, {
    headers: { "User-Agent": "AbasteceVotu/1.0" }
  });
  ...
  for (const posto of postos) {
    ...
    if (!coords) {
      ...
      if (updateError) {
        ...
        console.log(`✅ Salvo: lat=${coords.lat}, lng=${coords.lng}`);
      }
    }

    // Aguarda 1.1s entre requisições (limite do Nominatim)
